    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
               ...
               if (!response.ok) {
                throw new Error("Failed to fetchinterpretation");
               }
...
<div>
        ...
        <form onSubmit={handleSubmit} className="flex gap-3 flex-col"> 
            ...
            <textarea name="interpretation" rows={4} placeholder="Interpretation" value={formData.interpretation} 
            onChange={handleInputChange} className="py-1
            px-4 border rounded-md resize-none"></textarea>
            <button className="bg-black text-white mt-5 px-4 py-1 rounded-md">{isLoading ? "Updating..." : "Update Interpretation"}</button>
        </form>
